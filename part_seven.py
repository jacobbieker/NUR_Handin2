...
class Particle(object):
    def __init__(self, mass, position, node=None):
        self.mass = mass
...
class QuadTree(object):
    ...
    def make_quadrants(self, node):
        x_len = node.x_range[1] - node.x_range[0]
        y_len = node.y_range[1] - node.y_range[0]
    ...
    def add_particle(self, particle, node=None):
        ...
        if node.is_leaf:
            ...
            if len(node.particle_set) > 0:
                node_particle = node.particle_set[0]
                node.particle_set = ()
                m, com = self.add_particle(node_particle, node)
                masses.append(m)
            ...
            for index in range(len(masses)):
                tmp_x += masses[index]*positions[index][0]
